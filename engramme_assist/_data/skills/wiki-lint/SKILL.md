...
4. **Run the graph analysis once** and keep its output for the whole session:
   ```bash
   python3 "$OBSIDIAN_VAULT_PATH/_meta/scripts/insights.py" --json
   ```
   It returns one JSON object: `{orphans: [paths], backlinks: {path: count}, broken_links: [{source, target}], link_counts: {path: {in, out}}, cooccurrence_candidates: [{a, b, weight}]}`.

### Programmatic-first (framework rule)

The mechanical graph checks are **computed by the script, not hand-crawled** — this is the framework's programmatic-first rule (see `AGENTS.md` → Data Lifecycle). Read these fields from the `insights.py --json` output above:

- **Check 1 (orphans)** ← `orphans`
- **Check 2 (broken wikilinks)** ← `broken_links`
- **Incoming-link / hub counts** (Checks 7, 12c) ← `backlinks` / `link_counts`
- **Check 11 (synthesis-gap co-occurrence)** ← `cooccurrence_candidates`

Check 8 (tag-cluster cohesion) needs the intra-group adjacency the `--json` contract does not expose, so it stays LLM-computed — but take its "incoming wikilink count" inputs from `backlinks` rather than recounting.

Your job on these is to **interpret** the numbers and write the prose findings — never to re-count links with your own greps. If `insights.py` errors or is absent, **flag it and stop those checks** — do not silently fall back to manual crawling (that defeats the rule). The remaining checks (frontmatter, provenance fractions, contradictions, visibility, relationships) stay LLM-driven.
...
**How to check:**
- Read the `orphans` list from the `insights.py --json` output (Before You Start, step 4). Do not re-glob or re-grep — the script already excludes `index.md`/`log.md` and computes the graph.
...
**How to check:**
- Read the `broken_links` list from the `insights.py --json` output — each entry is `{source, target}` (the page holding the dangling link and the missing target). Do not re-grep for `[[...]]` yourself.
...
- Apply these thresholds:
  ...
  - **Hub pages** (top 10 by incoming wikilink count — take these from the script's `backlinks`, don't recount) with INFERRED > 20%: flag as "high-traffic page with questionable provenance" — errors on hub pages propagate to every page that links to them
...
**How to check:**
- Read `cooccurrence_candidates` from the `insights.py --json` output — each entry is `{a, b, weight}`, where `weight` is the co-occurrence strength (pages that link to both concepts) the script computed. Do not re-grep pairs by hand.
- Confirm the pair has no existing `synthesis/` page connecting `a` and `b` (check `synthesis/` titles and `[[wikilinks]]`); drop any that do.
- Flag the remaining pairs with `weight ≥ 3`.
...
1. Run all 13 lint checks (Checks 1–13 above).
...
- **Promote `draft` → `reviewed`:** pages where `lifecycle: draft` AND `created` > 30 days ago AND `base_confidence > 0.7`. Set `lifecycle: reviewed` and `lifecycle_changed: <today>` — nothing else. Do **not** write a `lifecycle_reason` (or any "auto-promoted by …" prose): per `AGENTS.md` the who/how of an edit is not knowledge.
...
Only after `--consolidate` wrote vault markdown, and only if `$QMD_WIKI_COLLECTION` is set (else skip). QMD is an index, not the source of truth — on failure report status but never roll back vault writes.

```bash
${QMD_CLI:-qmd} update && ${QMD_CLI:-qmd} embed   # embed only if vectors are stale
${QMD_CLI:-qmd} ls "$QMD_WIKI_COLLECTION"          # verify the collection
```
Report one status line: `QMD refreshed: update [+ embed] + verified` · `QMD skipped: <reason>` · `QMD failed: <error>`.