...
| `lifecycle: disputed` | `(DISPUTED[, marked <lifecycle_changed>][ — conflicts with [[other-page]]])` |
...
For a **DISPUTED** page the *reason* comes from the page itself, not a metadata field: read the page's `relationships: contradicts` entry or a `> ⚠️ Contradiction` callout in the body and name the conflicting page (`— conflicts with [[other-page]]`). If neither names a counterpart, annotate `(DISPUTED)` with no reason — never invent one. Include `marked <lifecycle_changed>` only if that date field is present.
...
[[disputed-page]] (DISPUTED, marked 2026-04-30 — conflicts with [[new-source]]) — Earlier said Y, now uncertain.
[[old-page]] (ARCHIVED: superseded by [[new-page]]) — Use the successor.
```

Pages with no lifecycle field (legacy pages predating the schema) are treated the same as `draft` — annotate if stale, skip otherwise.
