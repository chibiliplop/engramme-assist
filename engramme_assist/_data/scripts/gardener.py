...
# Derived views regenerated by script — same status as index.md (not a knowledge
# page, never carries lifecycle fields), excluded from review_due_coverage.
DERIVED_PAGES = {"projects/_portfolio.md"}
RAW_PROMOTED_TTL_DAYS = 7
RAW_UNPROMOTED_FLAG_DAYS = 14
ARCHIVE_GRACE_DAYS = 60
# _raw/morning-brief/ staging is always absorbed same-day by the brief pipeline,
# so it is purged on the promoted TTL regardless of manifest tracking.
RAW_BRIEF_SUBDIR = "morning-brief"
# hot.md ## Recent Activity windowing (mechanical trim; the prose stays LLM-written).
HOT_WINDOW_DAYS = 7
HOT_MAX_BULLETS = 15
...
def raw_age_days(path, name):
    """Age in days by the safer (older) of the file mtime and a leading YYYY-MM-DD name."""
    oldest = date.fromtimestamp(os.path.getmtime(path))
...
def raw_ttl(apply, manifest_text, entities_aliases):
    ...
    if not os.path.isdir(raw_dir):
        return {"deleted": [], "flagged": [], "archived": [], "brief_deleted": []}
    # morning-brief staging is absorbed same-day → purge on the promoted TTL,
    # by the safer/older of mtime & filename date, regardless of manifest tracking.
    brief_deleted = []
    brief_dir = os.path.join(raw_dir, RAW_BRIEF_SUBDIR)
        ...
        for root, _, files in os.walk(brief_dir):
            for name in sorted(files):
                if not name.endswith(".md"):
                    continue
                path = os.path.join(root, name)
                if raw_age_days(path, name) > RAW_PROMOTED_TTL_DAYS:
                    brief_deleted.append(os.path.relpath(path, VAULT).replace(os.sep, "/"))
                    if apply:
                        os.remove(path)
    now_flagged = set()
    for root, dirs, files in os.walk(raw_dir):
        dirs[:] = [d for d in dirs if os.path.join(root, d) != brief_dir]  # swept above
        for name in sorted(files):
...
def hot_window(apply):
    """Fenêtre glissante sur hot.md ## Recent Activity : purge les puces trop vieilles
    et plafonne la liste. Trim purement mécanique — ne touche ni le frontmatter ni les
    autres sections. Écrit sous --apply ; en dry-run, prévisualise (compte + word count)."""
    path = os.path.join(VAULT, "hot.md")
    if not os.path.exists(path):
        return {"trimmed": 0, "kept": 0, "word_count": 0}
    text = open(path, encoding="utf-8").read()
    lines = text.split("\n")
    start = next((i for i, l in enumerate(lines) if l.strip() == "## Recent Activity"), None)
    if start is None:
        return {"trimmed": 0, "kept": 0, "word_count": len(text.split())}
    end = len(lines)
    ...
    for b in bullets:
        ...
        if m:
            ...
            if d and (TODAY - d).days > HOT_WINDOW_DAYS:
                continue  # trop vieux : hors fenêtre
        kept.append(b)
    kept = kept[:HOT_MAX_BULLETS]
    trimmed = len(bullets) - len(kept)
    new_text = "\n".join(lines[:start + 1] + kept + [""] + lines[end:])
    if apply and trimmed:
        open(path, "w", encoding="utf-8").write(new_text)
    return {"trimmed": trimmed, "kept": len(kept), "word_count": len(new_text.split())}
...
def check():
    ...
    missing_rd = [rel for rel, fm, _ in pages
                  if rel.split("/")[0] in CATEGORY_DIRS and rel not in DERIVED_PAGES
                  and not fm.get("review_due")]
...
def main():
    ...
    print(json.dumps({
        ...
        "raw": raw,
        "raw_brief_deleted": len(raw.get("brief_deleted", [])),
        "hot_trimmed": hot["trimmed"],
        "hot_kept": hot["kept"],
        "hot_word_count": hot["word_count"],
